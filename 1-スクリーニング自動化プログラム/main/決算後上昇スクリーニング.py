...
# - INITIAL_MOMENTUM / STEADY_UP / BOTTOM_REVERSAL / QUALITYを独立strategy scoreとして保持。
# - STEADY_UPは日足のMA配列・傾き・MA25上滞在・安値切上げ・押しの浅さを評価し、瞬間急騰を主条件にしない。
...
def _live_steady_score(features: dict, current_price, turnover_oku, day_return_pct) -> tuple[float, bool, list[str]]:
    """急騰そのものではなく継続上昇構造を0-100で採点。"""
    f = features or {}
    ...
    ret20 = _live_num(f.get("ret20_pct"))
    score = 0.0; reasons=[]
    if ma5 is not None and ma25 is not None and ma5 > ma25: score += 15; reasons.append("MA5>MA25")
    if ma25 is not None and ma75 is not None and ma25 > ma75: score += 10; reasons.append("MA25>MA75")
    if s25 is not None and s25 > 0: score += 15; reasons.append("MA25上向き")
    if s75 is not None and s75 > 0: score += 5; reasons.append("MA75上向き")
    if stay is not None and stay >= 0.70: score += 15; reasons.append(f"MA25上{stay:.0%}")
    if lowrise is not None and lowrise >= 0.60: score += 10; reasons.append("安値切上げ")
    if up5 >= 3: score += 10; reasons.append(f"5日上昇{up5}日")
    if s5 is not None and s5 > 0: score += 10; reasons.append("MA5上向き")
    if hup > 0: score += 5; reasons.append("高値更新")
    not_overheated = (dr is None or dr <= LIVE_STEADY_MAX_DAY_RETURN_PCT) and (ret20 is None or ret20 <= 30.0)
    if not_overheated: score += 5; reasons.append("非過熱")
    gate = bool(
...
def _live_capture_snapshots(rows: list[dict], daily_map: dict[str, dict]) -> tuple[str, str, int]:
    ...
    for r in rows:
        ...
        vals.append((
            ...
            _live_num(_live_get(r,"現在値_raw","現在値","current_price")),
        ...
        if len(lows):
            ...
            for idx,val in lows.items():
                if last_val is None or float(val) < float(last_val)-1e-12:
                    ...
                    last_val=float(val)
        feat["minutes_since_low_update"] = round(float((nowdt-last_low_change_dt).total_seconds()/60.0),2) if last_low_change_dt is not None else None
        feat["low_update_stopped"] = bool(feat["minutes_since_low_update"] is not None and feat["minutes_since_low_update"] >= 20.0)
        out[str(code)]=feat
    return out
...
def _live_quality_score(row: dict) -> tuple[float,bool]:
    # tri_safetyは既存財務/需給安全評価の一部。正式なQUALITY専用モデルが未実装のため暫定proxyとして分離表示する。
    q=_live_num(_live_get(row,"tri_safety"))
    if q is None:
        return 0.0, False
    return round(max(0.0,min(100.0,q)),1), bool(q>=70.0)
...
def _live_candidate_payload(conn: sqlite3.Connection, rows: list[dict]) -> dict:
    ...
    candidates=[]; counts={k:0 for k in ("INITIAL_MOMENTUM","STEADY_UP","BOTTOM_REVERSAL","QUALITY","FUNDAMENTAL_EARLY","THEME_DISCOVERY","PRE_EARNINGS","POST_EARNINGS")}
    for r in rows:
        ...
        d=daily.get(code,{})
        steady_score, steady_on, steady_reason=_live_steady_score(d,current,turn,dr)
        ...
        quality_score,quality_on=_live_quality_score(r)
        ...
        if quality_on: active.append("QUALITY"); reasons.append("品質スコア高")
        if not active: continue
        for s in active: counts[s]+=1
        strategy_scores=[scores[{"INITIAL_MOMENTUM":"initial_momentum","STEADY_UP":"steady_up","BOTTOM_REVERSAL":"bottom_reversal","QUALITY":"quality"}[s]] for s in active]
        ...
        candidate={
            ...
            "intraday":{
                ...
                "max_pullback_pct":_live_num(intr.get("max_pullback_pct")), "minutes_since_low_update":_live_num(intr.get("minutes_since_low_update")),
            },
            "flags":{
    ...
    return {
        ...
        "stats":{
            "universe_count":len(codes), "snapshot_rows":snapshot_rows,
            "initial_momentum_count":counts["INITIAL_MOMENTUM"], "steady_up_count":counts["STEADY_UP"],
            "bottom_reversal_count":counts["BOTTOM_REVERSAL"], "quality_count":counts["QUALITY"],
...
def export_live_candidate_feed(conn: sqlite3.Connection, rows_or_df) -> bool:
    ...
    try:
        ...
        print(
            ...
            f"QUALITY={st.get('quality_count',0)} path={LIVE_CANDIDATE_FEED_PATH}"
        )
        return True
    except Exception as e:
