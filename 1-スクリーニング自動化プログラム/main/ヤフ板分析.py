...
def calculate_target_period(base_datetime=None):
    """
    実行時間がザラ場中なら「直近1時間」、それ以外は「前営業日〜現在」を計算
    """
    ...
    # 時刻を整数化 (例: 9時00分 -> 900, 15時30分 -> 1530)
    time_val = current_hour * 100 + current_minute
    
    # 市場時間の判定 (月〜金 09:00〜15:30)
    if 0 <= current_weekday <= 4 and 900 <= time_val <= 1530:
        if current_minute == 0: # ログがうるさいので最初だけ表示
            print(f"★ 【ザラ場モード】直近1時間分を取得します")
        start_target = base_datetime - timedelta(hours=1)
    ...
    if current_weekday == 0:    # 月曜日 -> 木曜の引け
        start_date = current_date - timedelta(days=1)
    elif current_weekday == 1:  # 火曜日 -> 金曜の引け
        start_date = current_date - timedelta(days=1)
    elif current_weekday == 2:  # 水曜日 -> 月曜の引け
        start_date = current_date - timedelta(days=1)
    elif current_weekday == 3:  # 木曜日 -> 火曜の引け
        start_date = current_date - timedelta(days=1)
    elif current_weekday == 4:  # 金曜日 -> 水曜の引け
        start_date = current_date - timedelta(days=1)
    elif current_weekday == 5:  # 土曜日 -> 木曜の引け
        start_date = current_date - timedelta(days=1)
    elif current_weekday == 6:  # 日曜日 -> 木曜の引け
...
def fetch_yahoo_bbs_market_adaptive(code, verbose=False):
    ...
    for loop in range(1000):
        ...
        params = {'code': code, 'size': '20', 'sort': '1'}
        if current_mid:
...
def analyze_bbs_manipulation(df, stock_name="不明"):
    """
    【究極版マルチ複合判定】全16パターン＋文脈理解（セマンティック処理）＋引用・アンケートスルー
    """
    if df.empty:
    ...
    for uid, u_df in df.groupby('userId'):
        ...
        if u_posts_count >= 5 or (u_posts_count / len(df) > 0.10): reasons.append(f"【高密度連投工作】短期間に {u_posts_count} 件の視覚占有工作")
    ...
    if not anomaly_df.empty:
        ...
        report['summary'].append(f"【警告】ボタン統計に異常スパイク（しきい値 {threshold:.1f}票超）の投稿を {len(anomaly_df)} 件検知。")
        
        for _, row in anomaly_df.iterrows():
            # ★判定の優先順位を整理：アンケートや質問を最優先でラベリングする
            if row['is_poll'] == 1:
                bias = "📊 掲示板アンケート（世論調査）"
                ...
                if row['sell_count'] > 0: fake_bear_score += (row['bad'] - row['good'])
                bias = "売り崩し（弱気の心理工作）"
                if row['sell_count'] > 0 and row['bad'] > row['good']: bias = "サクラ売り煽り（逆張りの安心ハメ工作）"
...
def generate_html_report(df, report, code, filepath):
    ...
    for a in report['anomalies']:
        bc = "badge-trap" if "サクラ" in a['bias'] or "隠蔽" in a['bias'] else ("badge-sell" if "売り" in a['bias'] else "badge-buy")
        if a.get('is_poll') == 1 or a.get('is_question') == 1:
            bc = "badge-watch"
        ...
        unified_list.append({
            ...
            'badge': f"<span class='badge {bc}'>🚨 スパイク: {a['bias']}</span>", 'body': a['body'],
...
def generate_ranking_html(ranking_list, filepath):
    ...
    for i, rep in enumerate(ranking_list):
        ...
        intent = rep.get('intent', '').split('】')[0] + '】' if '】' in rep.get('intent', '') else rep.get('intent', '')
    ...
    <head>
        ...
        <style>body {{ font-family: 'Helvetica Neue', Arial, sans-serif; background-color: #f0f2f5; color: #1e293b; margin: 0; padding: 30px; }} .header-main {{ display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 4px solid #ea580c; padding-bottom: 12px; margin-bottom: 20px; }} .header-main h1 {{ color: #ea580c; margin: 0; font-size: 26px; }} .meta-info {{ color: #64748b; font-size: 0.9em; }} .content-wrapper {{ background: white; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.04); }} table {{ width: 100%; border-collapse: collapse; margin-top: 10px; }} th {{ background-color: #1e3a8a !important; color: white !important; cursor: pointer; padding: 12px !important; text-align:center; }} td {{ padding: 10px; border-bottom: 1px solid #e2e8f0; }} tr:hover {{ background-color: #f8fafc; }}</style>
    </head>
    <body>
        <div style="max-width: 1200px; margin: 0 auto;">
            ...
            <div class="content-wrapper"><p style="font-size:14px; color:#475569;">※ <b>ヘッダーをクリックすると並び替えができます。</b> 投稿数が10件未満の銘柄は背景がグレーで表示されます。</p>
                <table id="rankTable" class="tablesorter">
                    <thead><tr><th>順位</th><th>銘柄名</th><th>コード</th><th>期待スコア🔥</th><th>投稿数</th><th>判定</th><th>需給フェーズ</th><th>大口の狙い</th><th>ファクト密度</th><th>最大流速</th></tr></thead>
                    <tbody>{table_rows}</tbody>
                </table>
            </div>
        </div>
        <script>$(document).ready(function() {{ $("#rankTable").tablesorter({{ theme: 'default', widgets: ['zebra'] }}); }});</script>
    </body>
    </html>
    """
    with open(filepath, 'w', encoding='utf-8') as f: f.write(html_content)
