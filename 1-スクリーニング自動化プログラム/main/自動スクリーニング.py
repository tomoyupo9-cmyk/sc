...
def _timed_daily_once(name, func, *args, **kwargs):
    ...
    today_str = dtm.date.today().strftime("%Y-%m-%d")
    ...
    if os.path.exists(marker_path):
        ...
        if last_run == today_str:
            print(f"[SKIP daily] {name} (already ok for {today_str})")
            return
            
    # 未実行なら _timed を使って計測実行
    ...
    try:
        ...
        print(f"[MARKER WARN] {name} のマーカー保存失敗: {e}")
...
def add_ai_analysis(conn, rows):
    ...
    print(f"[AI] {len(target_codes)}銘柄のAI分析を開始（超高速・一括計算モード）...")
    ...
    try:
        ...
        # --- 2. 個別銘柄のデータ準備 (SQLで銘柄順も保証) ---
        codes_str = "'" + "','".join(target_codes) + "'"
        hist_q = f"""
            SELECT コード, 日付, 始値, 高値, 安値, 終値, 出来高
            FROM price_history
            WHERE コード IN ({codes_str})
            ORDER BY コード ASC, 日付 ASC
        """
        df_hist_all = pd.read_sql(hist_q, conn)
        df_hist_all['日付'] = pd.to_datetime(df_hist_all['日付'])

        # ==============================================================
        # 【超高速化パッチ】 全銘柄のテクニカル指標を一括計算（ベクトル処理）
        # ==============================================================
        # groupbyで銘柄ごとに区切って計算
        grp = df_hist_all.groupby('コード', sort=False)
        
        # 移動平均・標準偏差
        df_hist_all['ma5'] = grp['終値'].transform(lambda x: x.rolling(5).mean())
        df_hist_all['ma20'] = grp['終値'].transform(lambda x: x.rolling(20).mean())
        df_hist_all['ma25'] = grp['終値'].transform(lambda x: x.rolling(25).mean())
        df_hist_all['ma75'] = grp['終値'].transform(lambda x: x.rolling(75).mean())
        df_hist_all['std20'] = grp['終値'].transform(lambda x: x.rolling(20).std())
        df_hist_all['ma_vol_5'] = grp['出来高'].transform(lambda x: x.rolling(5).mean())
        
        # RSI(14)の厳密な計算（元コードと完全一致＋ゼロ除算防止）
        delta = grp['終値'].diff()
        gain = delta.clip(lower=0)
        loss = -delta.clip(upper=0)
        
        gain_ma = gain.groupby(df_hist_all['コード'], sort=False).transform(lambda x: x.rolling(14).mean())
        loss_ma = loss.groupby(df_hist_all['コード'], sort=False).transform(lambda x: x.rolling(14).mean())
        
        rs = gain_ma / loss_ma
        df_hist_all['rsi_14'] = 100 - (100 / (1 + rs))
        # 無限大（ずっと上がり続けた場合）は100に、NaNは50にする
        df_hist_all['rsi_14'].replace([np.inf, -np.inf], 100, inplace=True)
        df_hist_all['rsi_14'] = df_hist_all['rsi_14'].fillna(50)

        # 各銘柄のデータ行数（75日あるかどうかの判定用）
        df_hist_all['count'] = grp.cumcount() + 1
        
        # 各銘柄の「最新日の行だけ」を抽出し、検索をゼロにする
        df_latest = df_hist_all.groupby('コード', sort=False).tail(1).set_index('コード')
        # ==============================================================

        # --- 3. 特徴量組み立て＆予測ループ（計算済みデータを引くだけ） ---
        ai_inputs = []
        valid_indices = []
        ...
        for i, r in enumerate(rows):
            ...
            # 最新データが存在するか、行数(データ期間)が十分かチェック
            if code not in df_latest.index:
                r['AIスコア'] = '-'
                r['AI判定'] = '-'
                r['AI目標値'] = '-' # 元コードの漏れを修正
                continue
                
            row_latest = df_latest.loc[code]
            if row_latest['count'] < 75: # データ不足
                r['AIスコア'] = '-'
                r['AI判定'] = '-'
                r['AI目標値'] = '-' # 元コードの漏れを修正
                continue

            # 最新の計算済みデータを取得（探す処理はゼロ）
            curr_o = row_latest['始値']
            curr_h = row_latest['高値']
            curr_l = row_latest['安値']
            curr_c = row_latest['終値']
            curr_v = row_latest['出来高']
            
            ma5 = row_latest['ma5']
            ma20 = row_latest['ma20']
            ma25 = row_latest['ma25']
            ma75 = row_latest['ma75']
            std20 = row_latest['std20']
            feat_rsi = row_latest['rsi_14']
            ma_vol_5 = row_latest['ma_vol_5']
            ...
            # RVOL
            feat_vol_ratio = curr_v / ma_vol_5 if (pd.notna(ma_vol_5) and ma_vol_5 > 0) else 1.0

            # 入力ベクトル（元コードと全く同じ構成）
            feats = [
                feat_return,                                
                (curr_h - curr_l) / curr_c if curr_c else 0, 
                (curr_c - curr_o) / curr_o if curr_o else 0, 
                (curr_h - max(curr_c, curr_o)) / curr_c if curr_c else 0, 
                (curr_c - ma5)/ma5 if pd.notna(ma5) and ma5 else 0,           
                (curr_c - ma25)/ma25 if pd.notna(ma25) and ma25 else 0,        
                (curr_c - ma75)/ma75 if pd.notna(ma75) and ma75 else 0,        
                feat_rsi,                                   
                (curr_c - ma20)/(2*std20) if pd.notna(std20) and std20 > 0 else 0, 
                feat_vol_ratio,                             
                1 if (pd.notna(ma5) and pd.notna(ma25) and pd.notna(ma75) and ma5>ma25 and ma25>ma75) else 0,       
                1 if (pd.notna(ma5) and pd.notna(ma25) and curr_c>ma5 and curr_c>ma25) else 0,   
                feat_market_return,                         
                feat_market_sentiment,                      
                feat_return - feat_market_return            
            ]
...
def _apply_shortterm_metrics(conn: sqlite3.Connection):
    ...
    try:
        ...
        q_hist = f"""
            ...
            FROM {hist_tbl}
            WHERE 日付 >= date((SELECT MAX(日付) FROM {hist_tbl}), '-120 days')
            ORDER BY コード, 日付
        """
        df_all = pd.read_sql_query(q_hist, conn, parse_dates=["日付"])
        if df_all.empty:
            return
        ...
        updates = []
        # 高速検索用に dict にする
        df_dict = {code: g for code, g in df_all.groupby("コード", sort=False)}
        ...
        for r in rows:
            ...
            if close_v is not None:
                ...
                except Exception: cur_close = _np.nan
            else:
...
def phase_yahoo_bulk_refresh(conn, codes, batch_size=100):
    ...
    """
    # === ★追加: 地方市場・エラー銘柄の除外フィルタ ===
    try:
        # 1. 毎回エラーになる特定コード（上場廃止済みなど）
        IGNORE_CODES = {"1738", "1777"}

        # 2. 除外したい市場キーワード（札証、名証、福証）
        # ※ここにキーワードを追加すれば、その市場は無視されます
        IGNORE_MARKETS = ["札", "札幌", "名", "名古屋", "福", "福岡", "S", "N", "F"]

        # DBから市場情報を取得してマッピング {コード: 市場名}
        cur = conn.cursor()
        cur.execute('SELECT コード, 市場 FROM screener')
        market_map = {str(row[0]): str(row[1] or "") for row in cur.fetchall()}
        cur.close()
        ...
        for c in codes:
            ...
            # (A) NGコードリストに含まれていたらスキップ
            if s_code in IGNORE_CODES:
                continue

            # (B) 市場名に「札」「名」「福」などが含まれていたらスキップ
            mkt = market_map.get(s_code, "")
            if any(kw in mkt for kw in IGNORE_MARKETS):
                continue
            
            # (C) コード末尾が .N .S .F ならスキップ（念のため）
            if s_code.endswith(".N") or s_code.endswith(".S") or s_code.endswith(".F"):
                continue

            filtered_codes.append(c)

        # ログ出し
        removed_count = len(codes) - len(filtered_codes)
        if removed_count > 0:
            print(f"[filter] 地方市場・NG銘柄を除外しました: {len(codes)} -> {len(filtered_codes)}件 (-{removed_count})")
        
        # 対象コードを入れ替え
        codes = filtered_codes

    except Exception as e:
        print(f"[filter] フィルタリング処理でエラー(無視して続行): {e}")
    # === ★追加ここまで ===
    ...
    for i in range(0, len(exist_codes), batch_size):
        ...
        try:
            df_wide = yf.download(safe_tickers, period="2d", interval="1d", group_by="ticker", threads=True, auto_adjust=False)
    ...
    for i in range(0, len(new_codes), batch_size):
        ...
        try:
            df_wide = yf.download(safe_tickers, period="12mo", interval="1d", group_by="ticker", threads=True, auto_adjust=False)
            df_add = _to_long_history(df_wide, tickers_map)
            total_added += _upsert_price_history(conn, df_add)
...
def compute_right_up_early_triggers(conn, as_of=None, log_datetime=None):
    ...
    # === 【一括計算パッチ】 ここで全銘柄のMAを一瞬で計算 ===
    grp = ph.groupby("コード", sort=False)
    ph["s10"] = grp["終値"].transform(lambda x: x.rolling(10).mean())
    ph["s20"] = grp["終値"].transform(lambda x: x.rolling(20).mean())
    ph["s50"] = grp["終値"].transform(lambda x: x.rolling(50).mean())
    ph["s100"] = grp["終値"].transform(lambda x: x.rolling(100).mean())
    ph["s200"] = grp["終値"].transform(lambda x: x.rolling(200).mean())
    ph["v20"] = grp["出来高"].transform(lambda x: x.rolling(20).mean())
    ph["hh60"] = grp["高値"].transform(lambda x: x.shift(1).rolling(HH_N, min_periods=HH_N).max())
    ...
    def _best_signal_vectorized(g):
        ...
        if not pd.isna(s10_t) and not pd.isna(hh60_t):
            ...
            down_vol_max = g["出来高"].where(down_mask).tail(POCKET_WIN).max()
        ...
        if not pd.isna(s200_t):
            ...
            if cond_d:
                ...
                vol_score = 0.0 if pd.isna(v20_t) else min(1.0, vol_t/max(1.0, v20_t))
                score = 25 + 45*near200 + 30*vol_score
                sigs.append((score, "200MAリクレイム", "cross&stay3d, 50MA↑,100MA↔↑"))
    ...
    if logs:
        try:
            cur.executemany("""
                ...
                VALUES(?,?,?,?)
                ON CONFLICT(コード, 日時, 種別) DO UPDATE SET 詳細 = excluded.詳細
            """, logs)
        except Exception:
...
def phase_signal_detection(conn: sqlite3.Connection):
    ...
    # バッチ処理（大きすぎるメモリ消費を防ぐため、500件ずつ一括処理）
    for i in range(0, len(codes), 500):
        ...
        up_ma = up.groupby(df['コード'], sort=False).transform(lambda x: x.rolling(14).mean())
        down_ma = down.groupby(df['コード'], sort=False).transform(lambda x: x.rolling(14).mean())
        rs = up_ma / down_ma.replace(0, 1e-9)
        df["RSI14"] = 100 - (100 / (1 + rs))
        
        # 52週高値 一括計算
        df["high_52"] = grp["終値"].transform(lambda x: x.rolling(252, min_periods=20).max())
        
        # vol60 一括計算
        df["vol60"] = grp["出来高"].transform(lambda x: x.rolling(60).mean())
        ...
        # ZIKA (時価総額) の一括取得
        c2 = conn.cursor()
        c2.execute(f"SELECT コード, 時価総額億円 FROM screener WHERE コード IN ({qmarks})", part)
        mcap_map = {str(r[0]): r[1] for r in c2.fetchall()}
        c2.close()
        ...
        for code, g in df.groupby("コード", sort=False):
            ...
            # --- 上昇余地スコア ---
            try:
                zika_oku = float(mcap_map.get(str(code)) or 0.0)
            except Exception:
                zika_oku = 0.0
            ...
            # --- 右肩上がりモメンタムスコア ---
            high_52 = last["high_52"] if pd.notna(last["high_52"]) else g["終値"].max()
            ...
            rumor_spike = (pd.notna(vol20) and pd.notna(last["出来高"]) and last["出来高"] >= vol20 * 1.5 and zenhi > 0)
            ...
            def _append_log(kind, score_value):
                log_rows.append((
                    ...
                    int(last['出来高']) if pd.notna(last['出来高']) else 0,
                    score_value,
                    0,
                    None, None, None, None,
                    None, None, None,
