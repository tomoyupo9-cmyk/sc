...
def __inject_inline_data_json(html: str, data_obj) -> str:
    ...
    try:
        ...
        dj = _json.dumps(data_obj, ensure_ascii=False, separators=(",", ":"), default=str)
    except Exception:
        dj = "{}"
...
import os as __os_patch_v8, logging as __lg_patch_v8, warnings as __wn_patch_v8, sys as __sys_patch_v8
...
# _str_series: 任意の配列/Seriesを安全に文字列Series化（NA→""）
try:
    _str_series  # type: ignore
except NameError:
    def _str_series(x):
        try:
            import pandas as pd
            if isinstance(x, pd.Series):
                try:
                    ...
                    s = x.astype(object).map(lambda v: "" if v is None else str(v))
                return s.fillna("")
            vals = ["" if v is None else str(v) for v in (list(x) if not isinstance(x, (str, bytes)) else [x])]
            s = pd.Series(vals)
            try:
                s = s.astype("string")
            except Exception:
...
try:
    _norm_code_str  # type: ignore
except NameError:
    def _norm_code_str(code):
        try:
            import re
            s = str(code).strip().upper()
            # よくあるサフィックスを除去
            for suf in (".T", "-T", ".JP", "-JP"):
                if s.endswith(suf):
                    s = s[: -len(suf)]
            # 数字以外を除去（例: "3350.T" -> "3350"）
            s = re.sub(r"[^0-9]", "", s)
            if not s:
                return ""
            # JP銘柄は通常4桁。5桁以上でもそのまま（新証券コード等に対応）
            if len(s) < 4:
                s = s.zfill(4)
            return s
        except Exception:
...
def _coerce_na_ok(s: pd.Series) -> pd.Series:
    # 数値/文字列混在でも落ちないように一旦文字列化。ただしNaNは維持
    if pd.api.types.is_string_dtype(s) or pd.api.types.is_categorical_dtype(s):
        return s
    return s.astype("string")

def s_eq(s: pd.Series, other: Any) -> pd.Series:
    """Series == other を安全・高速に。NaN, mixed dtype, list対応。"""
    if isinstance(other, (set, list, tuple, np.ndarray, pd.Series)):
        # membership 比較に変換
        if isinstance(other, pd.Series):
            other = other.dropna().unique()
        return _coerce_na_ok(s).isin(pd.unique(np.array(list(other), dtype=object)))
    # スカラ
    if other is None or (isinstance(other, float) and np.isnan(other)):
        return s.isna()
    # 文字列/数値混在でも比較可能に（NaNはFalse）
    left = _coerce_na_ok(s)
    right = str(other) if not isinstance(other, str) else other
    return left.fillna(pd.NA).eq(right).fillna(False)

def s_neq(s: pd.Series, other: Any) -> pd.Series:
    return ~s_eq(s, other)

def s_in(s: pd.Series, candidates: Iterable[Any]) -> pd.Series:
    return _coerce_na_ok(s).isin(pd.unique(np.array(list(candidates), dtype=object)))

def s_notin(s: pd.Series, candidates: Iterable[Any]) -> pd.Series:
...
def _sanitize_numbers(x):
    if isinstance(x, float):
        return None if not _math.isfinite(x) else x
...
def _get_yahoo_suffix_from_overrides(code4: str) -> str | None:
    ...
    見つからなければ None。
    対応テーブル候補やカラム名は広めに探索する（存在しない場合は握りつぶして次へ）。
    """
    # 4桁だけに正規化
    import re
    m = re.search(r"(\d{4})", str(code4))
    if not m:
        return None
    code4 = m.group(1)

    # 既存コード側に _get_db_conn() があれば使う（無ければ諦めて None）
    try:
        conn = _get_db_conn()  # noqa: F821  # 存在しない場合は except で握りつぶす
    except Exception:
        conn = None
    if conn is None:
        return None
    ...
    # サフィックス列候補（ドット有無どちらも想定）
    suffix_cols = ["yahoo_suffix", "yahoo_market_suffix", "yahoo_market", "yahoo_sfx"]

    cur = conn.cursor()
    for t in table_candidates:
        for c_code in code_cols:
            for c_sfx in suffix_cols:
                try:
                    # テーブル/カラム存在チェックを兼ねてクエリを直接投げる
                    cur.execute(f"SELECT {c_sfx} FROM {t} WHERE {c_code}=?", (code4,))
                    row = cur.fetchone()
                except Exception:
                    row = None
                if row and row[0]:
                    sfx = str(row[0]).strip().upper()
                    # 許容入力: "T" / ".T" / "TSE" など → ".T" に正規化
                    if sfx.startswith("."):
                        norm = sfx[:2]  # ".T"など先頭2文字
                    else:
                        # 代表的な表記を吸収
                        if sfx in ("T", "TSE", "TOKYO", "JPX", "PRIME", "STANDARD", "GROWTH", "MOTHERS", "JASDAQ"):
                            norm = ".T"
                        elif sfx in ("N", "NSE", "NAGOYA"):
                            norm = ".N"
                        elif sfx in ("S", "SSE", "SAPPORO"):
                            norm = ".S"
                        elif sfx in ("F", "FSE", "FUKUOKA"):
                            norm = ".F"
                        else:
                            # 1文字ならドット付与、想定外は一旦 ".T"
                            norm = f".{sfx[0]}" if len(sfx) == 1 else ".T"
                    return norm
    return None
...
def _ensure_links(d: dict) -> dict:
    ...
    # ここでサフィックス決定（override最優先）
    suffix = _resolve_yahoo_suffix(c4, market)

    # ===== 補完（既存値が無い場合のみ）=====
    d.setdefault("yahoo_url",       f"https://finance.yahoo.co.jp/quote/{c4}{suffix}")
    d.setdefault("kabutan_url",     f"https://kabutan.jp/stock/?code={c4}")
    d.setdefault("tradingview_url", f"https://jp.tradingview.com/symbols/TSE-{c4}/")
    d.setdefault("charts60_url",    f"./charts60/{c4}.html")
    d.setdefault("x_url",           f"https://x.com/search?q=%23{c4}%20OR%20{c4}%20株&f=live")
    return d
...
try:
    import orjson  # optional, fastest if available
    _HAS_ORJSON = True
except Exception:
...
def dumps_json_clean(obj,
    ...
    """
    import json, math, datetime, decimal, pathlib
    try:
        import numpy as _np
    except Exception:
        _np = None
    ...
    try:
        ...
        try:
            ...
            pass
        bs = _orjson.dumps(obj, default=_coerce, option=opt)
        s = bs.decode("utf-8")
        if ensure_ascii:
            s = s.encode("unicode_escape").decode("ascii")
        if indent or separators is not None:
            import json as _json
            s = _json.dumps(_json.loads(s), ensure_ascii=ensure_ascii, indent=indent, separators=separators)
        return s
    except Exception:
        return json.dumps(
            obj,
...
def resolve_yahoo_symbol(code: str,
    ...
    # === 追加ここから：TOPIXの素朴な別名吸収（DBに無いときの保険） ===
    alias = {'^TOPIX': '^TOPX', 'TOPIX': '^TOPX', '998405.T': '^TOPX'}
    if S in alias:
        return alias[S]
    # === 追加ここまで ===
    ...
    # 2) 規則解決
    # 2-1) 指数 or 既に .T 付き
    if S.startswith("^") or S.endswith(".T"):
        return s  # 元の表記で返す（大文字化しない）
    ...
    if len(S) == 4 and S.isdigit():
        ...
        if conn is not None:
            try:
                row = conn.execute(
                    "SELECT 市場 FROM screener WHERE コード=? LIMIT 1",
                    (s,)
                ).fetchone()
    ...
    if pat_3d1a.match(S) or (len(S) == 4 and pat_4_alnum.match(S) and not S.isdigit()):
        if try_online:
            # .T → 生 の順で“取れた方”を採用
            try:
                import yfinance as yf  # 局所import
                for cand in (s + ".T", s):
                    df = yf.download(cand, period="5d", interval="1d", progress=False)
                    if not df.empty:
                        return cand
            except Exception:
                pass  # 失敗時はオフラインフォールバック
        # オフライン or チェック失敗 → .T を優先
        return s + ".T"
...
def _ensure_latest_prices_index_rows(conn):
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO latest_prices(コード, 日付)
            SELECT DISTINCT
                s.コード,
        ...
        print("[WARN] _ensure_latest_prices_index_rows failed:", e)
    finally:
        try:
            cur.close()
        except Exception:
...
def _timed_daily_once(phase_name, fn, conn, *args, force=False, **kwargs):
    ...
    try:
        ...
        print(f"[OK] {phase_name} {_fmt_hms(time.time()-t0)}")
        return out
    except Exception as e:
...
def _override_set_none(conn, code_str: str):
    try:
        _ensure_override_table(conn)
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO yahoo_symbol_override(コード, 問い合わせシンボル) VALUES (?, 'NONE') "
            "ON CONFLICT(コード) DO UPDATE SET 問い合わせシンボル='NONE'",
            (str(code_str).strip(),)
        )
        conn.commit()
        print(f"[price-guard][INFO] override -> NONE (auto) for {code_str}")
    except Exception as e:
        print(f"[price-guard][WARN] override NONE failed for {code_str}: {e}")

def _probe_realtime_quote(sym: str) -> bool:
    """
    Try to obtain a current/last price using yfinance.fast_info (and yahooquery if available).
    Returns True if a plausible numeric price is found.
    """
    def _is_ok(val):
        try:
    ...
    try:
        ...
        t = _YQT(sym)
        qsum = t.summary_detail
        if isinstance(qsum, dict):
            d = qsum.get(sym) or next(iter(qsum.values()), {})
            for k in ("regularMarketPreviousClose", "regularMarketPrice", "ask", "bid"):
                v = d.get(k)
                if _is_ok(v):
                    return True
    except Exception:
...
def _override_set_auto_none_or_history(conn, code_str: str, sym_for_quote: str | None):
    """
    If realtime quote is available but history failed, set HISTORY_NONE (skip history only).
    Otherwise set NONE.
    """
    try:
        if sym_for_quote and _probe_realtime_quote(sym_for_quote):
            # history NG but realtime OK -> skip history only
            try:
                _ensure_override_table(conn)
                cur = conn.cursor()
                cur.execute(
                    "INSERT INTO yahoo_symbol_override(コード, 問い合わせシンボル) VALUES (?, 'HISTORY_NONE') "
                    "ON CONFLICT(コード) DO UPDATE SET 問い合わせシンボル='HISTORY_NONE'",
                    (str(code_str).strip(),)
                )
                conn.commit()
                print(f"[price-guard][INFO] override -> HISTORY_NONE (auto) for {code_str}")
            except Exception as e:
                print(f"[price-guard][WARN] override HISTORY_NONE failed for {code_str}: {e}")
        else:
            _override_set_none(conn, code_str)
    except Exception as e:
        print(f"[price-guard][WARN] override auto decision failed for {code_str}: {e}")
...
def _get_last_two_closes(conn, code: str):
    ...
    def _normalize_df(df: _pd.DataFrame) -> _pd.DataFrame:
        ...
        if hasattr(df, "index") and isinstance(df.index, _pd.MultiIndex):
        ...
        if hasattr(df, "index"):
            try:
                s = _pd.Series(df.index)
                s = _pd.to_datetime(s, utc=True, errors="coerce").dt.tz_convert("Asia/Tokyo").dt.date
                df = df.copy()
                df["日付"] = s.values
            except Exception:
                try:
                    s = _pd.to_datetime(df.index, errors="coerce").date
                    df["日付"] = s
                except Exception:
                    df["日付"] = _pd.to_datetime(df.index).date
        # 必須列フィルタ
        need = ["日付", "終値"]
    ...
    try:
        ...
        if df.empty:
            ...
            # 2) yfinance フォールバック
            if not fetched and _yf is not None:
                try:
                    for p in ("6mo", "3mo", "1mo"):
                        _hist = _yf.download(sym, period=p, interval="1d", auto_adjust=False, progress=False, threads=False)
                        _norm = _normalize_df(_hist)
                        if _norm is not None and not _norm.empty:
                            cur = conn.cursor()
                            for _, r in _norm.iterrows():
...
def _yahoo_quote_url(code: str, market: str | None = None, conn: sqlite3.Connection | None = None) -> str:
    ...
    優先度: 明示 market で推定 → resolve_yahoo_symbol() からサフィックス推定 → '.T' フォールバック
    """
    try:
        raw = ("" if code is None else str(code)).strip()
        if not raw:
            return ""
        c4 = raw.zfill(4) if raw.isdigit() else raw
        ...
        if suf is None and conn is not None:
            try:
                # resolve_yahoo_symbol で直接推定
                sym = resolve_yahoo_symbol(c4, conn, True)  # type: ignore[name-defined]
                if sym and isinstance(sym, str) and sym.upper().endswith(tuple([".T", ".N", ".S", ".F"])):
                    suf = "." + sym.rsplit(".", 1)[-1]
            except Exception:
                pass
        if suf is None:
...
# === [INJECTED] Vectorized minimum fields for DataFrames ===
def _vectorize_minimum_fields(df):
    # DataFrameに 'c4' / 'yahoo_url' / 'x_url' / 'chart_url' を付与
    try:
    ...
    if "コード" in df.columns:
        try:
            c4 = _pd.Series(df["コード"], copy=False).astype(str).str.strip().str.zfill(4)
            df["c4"] = c4
        except Exception:
            pass
    else:
    ...
    if "銘柄名" in df.columns:
        try:
            from urllib.parse import quote as __q
            df["x_url"] = "https://x.com/search?q=" + df["銘柄名"].astype(str).map(lambda s: __q(s))
        except Exception:
            try:
                df["x_url"] = "https://x.com/search?q=" + df["銘柄名"].astype(str)
            except Exception:
                pass
    ...
    # Yahoo URL
    try:
        df["yahoo_url"] = "https://finance.yahoo.co.jp/quote/" + df["c4"] + ".T"
    except Exception:
        pass

    # X URL（銘柄名があればそれで検索リンクを作る）
    if "銘柄名" in df.columns:
        try:
            from urllib.parse import quote as __q
            df["x_url"] = "https://x.com/search?q=" + df["銘柄名"].astype(str).map(lambda s: __q(s))
...
def _get_last_two_closes_safe(conn, code):
    """必ず4要素タプルで返す安全ラッパー。異常時は (None, None, None, None)。"""
    try:
        res = _get_last_two_closes(conn, code)
    except Exception as e:
        print(f"[live][WARN] _get_last_two_closes_safe: {e}")
        return (None, None, None, None)
    if not isinstance(res, tuple) or len(res) != 4:
        print("[live][WARN] apply-live: _get_last_two_closes returned invalid result")
        return (None, None, None, None)
    return res
...
def _run_eod_overwrite_pipeline(
    ...
    """単一ライターに作業を順番投入（history → screener → latest）。
    conn は使用せず、writer 側が保持する接続で実行する。
    """
    if history_refresher is None or screener_from_history is None:
        raise ValueError("_run_eod_overwrite_pipeline: 関数を渡してください")
    # enqueue in order
    enqueue_call(history_refresher.__name__)
    enqueue_call(screener_from_history.__name__)
    if sync_latest_prices is not None:
        enqueue_call(sync_latest_prices.__name__)
    # wait until finished
    wait_writes()
# === /価格計算の安全ガード ===
# === ライブ価格取得（場中のみ自動） ===
def _fetch_live_price_map(conn,codes):
    """
...
def _fetch_live_quote_map(conn,codes4):
    ...
    try:
        ...
        syms = [resolve_yahoo_symbol(str(c), conn) for c in codes4]
        yq = _YQ(syms, validate=True)
...
def _load_avg_turnover_map(conn, codes4, window: int = 20):
    ...
    cur = conn.cursor()
    cur.execute(f"""
        SELECT コード, 日付, 終値, 出来高,ROUND( (COALESCE(終値,0) * COALESCE(出来高,0)) / 100000000.0, 2 ) AS 代金億 FROM price_history
        WHERE コード IN ({ph})
        ORDER BY コード, 日付
    """, codes4)
    avg = {}
...
def _apply_live_overrides(conn,row: dict, q: dict, avg_turn_map: dict):
    ...
    if isinstance(px, (int, float)):
        try:
            _apply_price_fields(conn, row, code=str(row.get("コード") or ""), live_price=px)
        except NameError:
...
def _apply_shortterm_metrics(conn: sqlite3.Connection):
    ...
    ・signal_date/シグナル更新日 が無い銘柄（指数など）でも安全に更新
      - ATR_14 は履歴があれば通常計算
      - Rate_Since_Signal_High / Days_Since_Signal_High は
        シグナル日が無い場合、直近60日（営業日相当）で代替計算
    """
    import pandas as pd
    import numpy as _np
    ...
    try:
        ...
        for r in rows:
            ...
            if hdf.empty:
                ...
                # 既存の _calculate_atr_ewm があれば利用、無ければフォールバック
                atr14 = None
                try:
                    atr_series = _calculate_atr_ewm(hdf, 14)  # 既存関数想定
                    if len(atr_series):
                ...
                if atr14 is None:
                    ...
                    try:
                        ...
                        tr = _np.nanmax(
                            _np.vstack([
                                (hi - lo).to_numpy(_np.float64),
                                (hi - prev_cl).abs().to_numpy(_np.float64),
                                (lo - prev_cl).abs().to_numpy(_np.float64),
                            ]),
                            axis=0
                        )
                        tr = pd.Series(tr, index=hdf.index)
                        atr_series = tr.ewm(span=14, adjust=False, min_periods=1).mean()
                        atr14 = float(atr_series.iloc[-1]) if len(atr_series) else None
                    except Exception:
                        atr14 = None

            # ===== シグナル後モメンタム =====
            # signal_date（またはシグナル更新日）
            ...
            # どこから比較するか：シグナル日が無ければ直近60日
            if pd.isna(signal_date):
                q2 = f"""
                    SELECT 日付, 高値, 終値
                    FROM {hist_tbl}
                    WHERE コード = ?
                      AND 日付 >= date((SELECT MAX(日付) FROM {hist_tbl} WHERE コード = ?), '-60 days')
                    ORDER BY 日付
                """
                h2 = pd.read_sql_query(q2, conn, params=[code, code], parse_dates=["日付"])
            else:
                q2 = f"""
                    SELECT 日付, 高値, 終値
                    FROM {hist_tbl}
                    ...
                    ORDER BY 日付
                """
                h2 = pd.read_sql_query(q2, conn, params=[code, str(pd.to_datetime(signal_date).date())], parse_dates=["日付"])

            # max_high / idx_max（高値の最大とその日付）
            if h2.empty:
                max_high = _np.nan
                idx_max_dt = pd.to_datetime(ldate, errors="coerce")
            else:
                hi_num = pd.to_numeric(h2["高値"], errors="coerce")
                if hi_num.dropna().empty:
                    max_high = _np.nan
                    idx_max_dt = pd.to_datetime(ldate, errors="coerce")
                else:
                    max_high = float(hi_num.max())
                    # idxmax は Series のindex（行ラベル）。locで日付取得
                    idx_max_row = hi_num.idxmax()
                    idx_max_dt = pd.to_datetime(h2.loc[idx_max_row, "日付"], errors="coerce")

            # 現在値の決定：latestの値が無ければ、比較区間 or 全履歴の最終終値を使用
            if close_v is not None:
                try:
                    cur_close = float(close_v)
                except Exception:
                    cur_close = _np.nan
            else:
                if not h2.empty and pd.to_numeric(h2["終値"], errors="coerce").dropna().size > 0:
                    cur_close = float(pd.to_numeric(h2["終値"], errors="coerce").dropna().iloc[-1])
                elif not hdf.empty and pd.to_numeric(hdf["終値"], errors="coerce").dropna().size > 0:
                    cur_close = float(pd.to_numeric(hdf["終値"], errors="coerce").dropna().iloc[-1])
                else:
                    cur_close = _np.nan

            # 騰落率・経過日数（NaNはNoneへ）
            if not _np.isfinite(cur_close) or not _np.isfinite(max_high) or max_high == 0.0:
                rate_since = None
            else:
            ...
            if pd.isna(base_date) or pd.isna(idx_max_dt):
                ...
                try:
                    days_since = int((base_date - idx_max_dt).days)
                except Exception:
                    days_since = None
...
def phase_resistance_update(conn):
    ...
    # latest_prices に コード列があることを保証（旧スキーマ互換）
    _ensure_latest_prices_code_col(conn)
    # 市場='指数' のコードを latest_prices に足しておく（^N225, VIX など）
    _ensure_latest_prices_index_rows(conn)

    cur = conn.cursor()
    try:
        # latest_prices に存在するコードすべてについてサポレジ計算
...
_CODE_4DIGIT   = re.compile(r"^\d{4}$")
_CODE_3D1A     = re.compile(r"^\d{3}[A-Za-z]$")
_CODE_4ALNUM   = re.compile(r"^[A-Za-z0-9]{4}$")

def _normalize_code_for_insert(s: str) -> str | None:
    if s is None:
        return None
    t = str(s).strip()
    if not t:
    ...
    T = t.upper()
    # 国内コード系はそのまま（表記統一のみ）
    if _CODE_4DIGIT.fullmatch(T) or _CODE_3D1A.fullmatch(T) or _CODE_4ALNUM.fullmatch(T):
        return T
    # それ以外（指数/ETFの ^ や .T 付きなど）も“そのまま採用”
    return T
...
def phase_delist_cleanup(conn: sqlite3.Connection,
    ...
    if "コード" not in master.columns:
        print("CSVに列 'コード' が見つからないためスキップ:", master_csv_path)
        return
    ...
    if has_protect:
        ...
        protect_set = {str(row[0]).strip().upper() for row in cur.fetchall()}
    ...
    print(f"上場廃止による削除: {len(targets)} 件" + (f"（保護:{skipped_by_protect} 件）" if skipped_by_protect else ""))
    ...
    try:
        ...
        for c in codes:
            try:
                ...
                fi = yf.YQ(resolve_yahoo_symbol(c, conn, True)).fast_info
...
LEFT JOIN latest_prices lp
  ON CAST(lp.コード AS TEXT) = CAST(s.コード AS TEXT)
    ...
    symbols_all = [resolve_yahoo_symbol(c, conn) for c in codes]
...
def _derive_triangle_scores(d):
    ...
    # 合成スコアがあればベースにする（0〜100）
    base = _to_float(d.get("合成スコア") or d.get("推奨比率_raw"))
    if base is not None:
        g += base

    # 初動・右肩・早期・底打ちフラグ
    for key in ["初動フラグ", "右肩上がりフラグ", "右肩早期フラグ", "底打ちフラグ"]:
        if d.get(key) in ["○", "◯", "あり", 1, True]:
            g += 10
    ...
    # 空売り機関なし
    if d.get("空売り機関なし_flag") == 1:
        s += 15
    else:
        s -= 5

    # 割安業績
    if d.get("営利対時価_flag") == 1:
        s += 10

    # 営利YoY
    yoy = _to_float(d.get("営利YoY"))
    if yoy is not None:
        if yoy > 0:
            s += 5
        else:
    ...
    # ---------- 3) 短期ボラスコア（Vol ■） ----------
    v = 0  # ★必ずここで初期化するので、NameErrorが出なくなる
...
def __lru(maxsize=8192):
    return __lru_cache(maxsize=maxsize)

@__lru()
def _fmt_code4__fast(code):
    try:
...
@__lru()
def _chart_url__fast(c4):
    return f"/charts60/{c4}.html"
    
def _prepare_rows_fast(df: pd.DataFrame, conn):
    ...
    if "c4" not in _df.columns:
        ...
        _df["c4"] = _df.get("コード", "").map(_fmt4)

    if "chart_url" not in _df.columns:
        def _chart_from_c4(c4):
            return _chart_url__fast(c4) if " _chart_url__fast" in globals() else ""
        _df["chart_url"] = _df["c4"].map(_chart_url__fast if " _chart_url__fast" in globals() else (lambda _c4: ""))
    ...
    for r in _df.itertuples(index=False):
        ...
        try:
            ...
            try:
                c4_raw = getattr(r, "コード")
                c4_val = (str(c4_raw).zfill(4) if c4_raw is not None else "")
            except Exception:
                c4_val = ""

        try:
            name_val = getattr(r, "銘柄名")
        except Exception:
            name_val = ""

        # --- ここが重要：欠損は None、ある時は完全URL ---
        d["yahoo_url"] = (f"https://finance.yahoo.co.jp/quote/{c4_val}.T" if c4_val else None)

        try:
            from urllib.parse import quote as __quote
            d["x_url"] = (f"https://x.com/search?q={__quote(str(name_val))}" if name_val else None)
        except Exception:
            # もし urllib が無くても最低限の動作はさせる
            d["x_url"] = (f"https://x.com/search?q={name_val}" if name_val else None)
...
def _prepare_rows_safe(df, conn):
    try:
        return _prepare_rows_fast(df, conn)
    except Exception as __e:
        try: print(f"[rows][WARN] fast-path failed: {__e}", flush=True)
        except Exception: pass
        return _prepare_rows(df, conn)
# ==== [/FAST PATH HELPERS - REAL IMPL] ==================================
...
def preload_price_summaries(conn, codes, window_days=120):
    ...
    import datetime as _dt
    start_date = (_dt.date.today() - _dt.timedelta(days=window_days)).isoformat()
    out = {}
    CH = 700
    for i in range(0, len(codes), CH):
        ...
        """
        try:
            dfp = pd.read_sql_query(sql, conn, params=[*part, start_date], parse_dates=["日付"])
        except Exception as _e:
...
def _kabunews_make_url(code: str, name: str) -> str:
    code = (code or "").strip()
    name = (name or "").strip()
    q = f'site:kabutan.jp ({code} OR "{name}")'
    qs = _u.urlencode({
        "q": q,
...
def _kabunews_fetch(code: str, name: str) -> list[dict]:
    """Google News RSS (site:kabutan.jp) から該当コード/銘柄のニュースを取得。
    デバッグ用の詳細 print を追加。"""
    import time as _t
    key = f"{str(code).zfill(4)}:{(name or '').strip()}"
    dbg = bool(_KABUNEWS_CONF.get("debug", True))  # 必要なら False に

    if key in _KABUNEWS_CACHE:
        if dbg:
            print(f"[kabunews] CACHE HIT {key} items={len(_KABUNEWS_CACHE[key])}", flush=True)
        return _KABUNEWS_CACHE[key]

    url = _kabunews_make_url(code, name)
    t0 = _t.perf_counter()
    try:
        import requests as _rq, xml.etree.ElementTree as _ET
        headers = {"User-Agent": _KABUNEWS_CONF["user_agent"]}
        r = _rq.get(url, headers=headers, timeout=_KABUNEWS_CONF["http_timeout"])
        ct = r.headers.get("Content-Type", "")
        if dbg:
            print(
                f"[kabunews] RESP {key} status={r.status_code} ct='{ct}' "
                f"bytes={len(r.content)} dt={(_t.perf_counter()-t0):.2f}s",
                flush=True
            )
        r.raise_for_status()

        # RSSでないっぽい時は先頭だけ出す（Cloud対策/HTMLページ検知用）
        if "xml" not in ct.lower() and dbg:
            head = r.text[:160].replace("\n", " ") if isinstance(r.text, str) else ""
            print(f"[kabunews][WARN] non-XML response {key} head={head!r}", flush=True)

        root = _ET.fromstring(r.text)

    except Exception as e:
        if dbg:
            # 文字化け回避のため repr にして短縮
            try_head = ""
            try:
                try_head = (r.text[:160].replace("\n", " ")
                            if 'r' in locals() and isinstance(r.text, str) else "")
            except Exception:
                pass
            print(
                f"[kabunews][ERR] fetch {key}: {type(e).__name__}: {e!s} head={try_head!r}",
                flush=True
            )
        _KABUNEWS_CACHE[key] = []
        return []
    ...
    try:
        for it in root.findall(".//item"):
            ...
            if title and link:
                items.append({"title": _html.unescape(title), "url": link, "pub": pub})
            if len(items) >= int(_KABUNEWS_CONF.get("max_items_per_symbol") or 3):
                break
        if dbg:
            print(f"[kabunews] PARSED {key} items={len(items)}", flush=True)
    except Exception as e:
        if dbg:
            print(f"[kabunews][ERR] parse {key}: {type(e).__name__}: {e!s}", flush=True)
        items = []

    # ★ここから追加：pub 日付で降順ソート（新しい順）
    try:
        ...
        def _parse_pub_dt(s: str):
            ...
            if not s:
                return None
            # まず RFC822 形式 (Wed, 19 Nov 2025 07:30:00 GMT) を試す
            try:
                return _pdt(s)
            except Exception:
                pass
        ...
        items.sort(
            key=lambda it: (_parse_pub_dt(it.get("pub")) or _dt.min),
            reverse=True,  # 新しい順
        )
    except Exception as e:
        if dbg:
            print(f"[kabunews][WARN] sort by pub failed {key}: {type(e).__name__}: {e!s}",
                  flush=True)
    # ★追加ここまで

    # 追加: BERT スコアリング（有効時のみ）
    if items and _KABUNEWS_CONF.get("enable_bert", True):
        if dbg:
            print(
                f"[kabunews] BERT start {key} n={len(items)} "
                f"model={_KABUNEWS_CONF.get('bert_model')}",
                flush=True
            )
        try:
            bert_t0 = _t.perf_counter()
            from transformers import AutoTokenizer, AutoModelForSequenceClassification, pipeline
            _tok = AutoTokenizer.from_pretrained(_KABUNEWS_CONF["bert_model"])
            _mdl = AutoModelForSequenceClassification.from_pretrained(_KABUNEWS_CONF["bert_model"])
            _pipe = pipeline("sentiment-analysis", model=_mdl, tokenizer=_tok, device=-1)

            texts = [it["title"] for it in items]
            preds = _pipe(texts)

            pos_cnt = neg_cnt = other_cnt = 0

            for it, pr in zip(items, preds):
                label = str(pr.get("label") or "")
                ...
                if ("POS" in s_norm) or ("ポジ" in s_norm):
                    pos_cnt += 1
                elif ("NEG" in s_norm) or ("ネガ" in s_norm):
                    neg_cnt += 1
                else:
                    other_cnt += 1
            ...
            if dbg:
                print(
                    f"[kabunews] BERT done {key} total={len(preds)} "
                    f"pos={pos_cnt} neg={neg_cnt} other={other_cnt} "
                    f"dt={(_t.perf_counter()-bert_t0):.2f}s",
                    flush=True
                )
            ...
            if dbg:
                print(
                    f"[kabunews][WARN] BERT skipped {key}: {type(e).__name__}: {e!s}",
                    flush=True
                )
            for it in items:
                it["sentiment"] = None
    ...
    elif items and not _KABUNEWS_CONF.get("enable_bert", True):
        if dbg:
            print(f"[kabunews] BERT disabled by config for {key}", flush=True)

    _KABUNEWS_CACHE[key] = items
    if dbg:
        print(
            f"[kabunews] DONE {key} items={len(items)} "
            f"total_dt={(_t.perf_counter()-t0):.2f}s",
            flush=True
        )
    return items
...
def kabutan_news_fetch_bulk(pairs, per_symbol=None):
    ...
    """
    import requests as _rq, xml.etree.ElementTree as ET
    per_symbol = per_symbol or int(_KABUNEWS_CONF.get("max_items_per_symbol", 3))
    # chunk pairs to keep URL length <= 1800 chars (safe margin) and <= 8 symbols per request
    out = {str(c).zfill(4): [] for c,_ in pairs if c}
    CHUNK = 8
    i = 0
    while i < len(pairs):
        ...
        # fallback if URL too long: reduce chunk size dynamically
        if len(url) > 1800 and len(chunk) > 1:
            # halve chunk and push remainder back
            i -= CHUNK  # rewind
            CHUNK = max(1, CHUNK//2)
            if CHUNK == 1:
                # accept long single if unavoidable
                chunk = pairs[i:i+1]
                i += 1
                url = _kabunews_make_multi_url(chunk)
            continue
        try:
            resp = _rq.get(url, headers={"User-Agent": _KABUNEWS_CONF["user_agent"]}, timeout=_KABUNEWS_CONF["http_timeout"])
            if resp.status_code != 200:
                _log(f'[kabunews][bulk][WARN] status={resp.status_code} url={url[:160]}')
                continue
            root = ET.fromstring(resp.content)
            items = root.findall(".//item")
            # prepare quick matchers
            def key(code, name):
                return str(code).zfill(4), (str(name) or '').strip()
            ck = [key(c,n) for c,n in chunk]
            # simple assign: if code or name appears in title or link, bucket it
            for it in items:
                title = (it.findtext("title") or "").strip()
                link = (it.findtext("link") or "").strip()
            ...
            for c4 in list(out.keys()):
                ...
                deduped.sort(
                    key=lambda tpl: (_parse_pub_dt(tpl[2]) or _dt.min),
                    reverse=True,
                )
...
def kabutan_news_lines_bulk_for_dataframe(df, code_col="コード", name_col="銘柄名", per_symbol=None):
    """
    df(コード・銘柄名入り) → 株探ニュース(3) 用の Series を返す
    """
    if df is None or getattr(df, "empty", True):
        import pandas as _pd
    ...
    def _fmt_pub(pub: str) -> str:
        if not pub:
            return ""
        s = str(pub).strip()
        for pat in ("%a, %d %b %Y %H:%M:%S %Z", "%a, %d %b %Y %H:%M:%S %z"):
            try:
                dt = datetime.strptime(s, pat)
                return dt.strftime("%y/%m/%d")  # ← 25/11/14 みたいな形
            except Exception:
                pass
        if len(s) >= 10 and s[0:4].isdigit() and s[4] in "-/" and s[7] in "-/":
            core = s[:10].replace("-", "/")   # 2025/11/14
            return core[2:]                   # → 25/11/14
        return ""
    ...
    for idx, code4 in idx_map:
        data.append(fmt(code4, result_map.get(code4, [])))

    return _pd.Series(data, index=df.index, dtype=object)

def kabutan_news_line(code: str, name: str) -> str:
    rows = _kabunews_fetch(code, name)
    if not rows:
        return ""
    def _fmt(it):
        s = it["title"]
        if it.get("sentiment"):
            s = f'[{it["sentiment"]}] ' + s
        return s
    return " ｜ ".join(_fmt(it) for it in rows)
...
def _ensure_news_cols_both():
    try:
        globals()['df_cand'] = ensure_news_cols(globals().get('df_cand'))
    except Exception:
        pass
    try:
        globals()['df_all'] = ensure_news_cols(globals().get('df_all'))
    except Exception:
        pass
...
def _ensure_theme_tables(conn: sqlite3.Connection):
    ...
    try:
        ...
        try:
            print("[theme][WARN] _ensure_theme_tables failed:", e)
        except Exception:
            pass
    finally:
        try:
...
def _update_kabutan_theme_ranking(conn: sqlite3.Connection,
                                  url: str = "https://kabutan.jp/info/accessranking/3_2"):
    """
    Kabutan「人気の株式テーマ 3日間アクセスランキング」から
    テーマ名と同ページに掲載されている関連銘柄コードだけを取得して
    theme_master / stock_theme_kabutan に反映する。
    ...
    try:
        ...
        try:
            print("[theme][WARN] fetch kabutan themes failed:", e)
        except Exception:
            pass
        return
    ...
    try:
        ...
        # 2) ランキングテーブルを特定
        #    現在のレイアウトでは <div class="acrank acrank_theme"><table>...</table> が
        #    テーマランキング本体になっている。
        container = soup.find("div", class_="acrank_theme")
        if container is None:
            # 念のため class 完全一致でも試す
            container = soup.find("div", class_="acrank acrank_theme")
        if container is None:
            try:
                print("[theme][WARN] acrank_theme container not found; skip update")
            except Exception:
                pass
            return
        ...
        if table is None:
            try:
                print("[theme][WARN] acrank_theme table not found; skip update")
            except Exception:
                pass
            return

        # 3) 行ごとに テーマ名＋銘柄コードを抽出
        for tr in table.find_all("tr"):
            # テーマ名は <td class="acrank_url"><a>テーマ名</a></td>
            ttd = tr.find("td", class_="acrank_url")
            if ttd is None:
                continue

            tname = (ttd.get_text(" ", strip=True) or "").strip()
            if not tname:
                continue

            # 明らかにノイズな見出しは除外
            if "関連サイト" in tname or "外部リンク" in tname:
                continue

            # theme_master upsert
            cur.execute(
                "INSERT OR IGNORE INTO theme_master(theme_name) VALUES (?)",
                (tname,)
            )
            
            cur.execute(
                "SELECT theme_id FROM theme_master WHERE theme_name = ?",
                (tname,)
            )
            row = cur.fetchone()
            if not row or row[0] is None:
                # theme_id が取れない異常系。念のためスキップ
                continue
            theme_id = int(row[0])


            # 同じ行に並んでいる <a href="...stock...?code=XXXX"> から銘柄コードを拾う
            codes = set()
            for a in tr.find_all("a", href=True):
                href = a["href"] or ""
                m = _re.search(r"code=(\d{4}[A-Z]?)", href)
                if not m:
                    m = _re.search(r"/stock/(\d{4}[A-Z]?)", href)
                if not m:
                    continue
                codes.add(m.group(1))

            for c in codes:
                # 4桁の数字部分だけを採用（285A → 0285 のような扱いはここではせず、数値部のみ）
                m2 = _re.match(r"(\d{4})", str(c))
                if not m2:
                    continue
                c4 = m2.group(1)

                cur.execute(
                    "INSERT OR REPLACE INTO stock_theme_kabutan(コード, theme_id, 取得日) "
                    "VALUES (?, ?, ?)",
                    (c4, theme_id, today)
                )
        ...
        try:
            print("[theme][WARN] parse/insert kabutan themes failed:", e)
        except Exception:
            pass
    finally:
        try:
...
def _load_theme_maps_for_screener(conn: sqlite3.Connection):
    ...
    try:
        cur = conn.cursor()
        for code4, tname in cur.execute(
            "SELECT printf('%04d', CAST(s.コード AS INTEGER)) AS c4, m.theme_name "
            "FROM stock_theme_kabutan s "
            "JOIN theme_master m ON s.theme_id = m.theme_id"
        ):
            if not code4 or not tname:
                continue
            kabutan_map.setdefault(str(code4), set()).add(str(tname))
...
def _attach_related_themes(df, kabutan_map, theme_names):
    ...
    try:
        ...
        codes = df["コード"].astype(str).map(lambda x: x.zfill(4))
...
def compute_relative_strength(df_all, index_df_map):
    """
    df_all: 全銘柄データ（現在の df_all）
    index_df_map: {指数コード: 指数データ（直近～過去の価格を含むDataFrame）}
    
    必要列：
        df_all      : "コード", "現在値", "前日終値", "終値20日前"
        index_df_map: 各指数 DataFrame に "close", "close_5ago", "close_20ago"
    """

    # RS列が既に存在する場合は上書き（coerce安全）
    df_all["RS_5"] = None
    df_all["RS_20"] = None
    ...
    # 指数の騰落率計算
    def idx_return(idx_df, days):
        try:
            c0 = float(idx_df["close"].iloc[-1])
            cp = float(idx_df[f"close_{days}ago"].iloc[-1])
            return (c0 - cp) / cp
        except Exception:
            return None

    # 銘柄の騰落率
    def stock_return(row, days):
        try:
            if days == 5:
                return (float(row["現在値"]) - float(row["終値5日前"])) / float(row["終値5日前"])
            else:
                return (float(row["現在値"]) - float(row["終値20日前"])) / float(row["終値20日前"])
        except Exception:
            return None
    ...
    for _, r in df_all.iterrows():
        idx_symbol = resolve_index_symbol(r)
        idx_df = index_df_map.get(idx_symbol)

        if idx_df is None:
            rs5_list.append(None)
            rs20_list.append(None)
            continue

        idx5 = idx_return(idx_df, 5)
        idx20 = idx_return(idx_df, 20)
        st5 = stock_return(r, 5)
        st20 = stock_return(r, 20)

        if st5 is not None and idx5 is not None:
            rs5_list.append(st5 - idx5)
        else:
            rs5_list.append(None)

        if st20 is not None and idx20 is not None:
            rs20_list.append(st20 - idx20)
        else:
            rs20_list.append(None)

    df_all["RS_5"] = rs5_list
...
def phase_export_html_dashboard_offline(conn, html_path, template_dir="templates",
    ...
    def _safe_jsonable(v):
        if v is None: return None
        try:
            if isinstance(v, (dtm.date, dtm.datetime)): return str(v)
        except Exception:
            pass
        try:
    ...
    # 1.5) テーマ関連テーブルを保証し、Kabutan 人気テーマを最新化
    try:
    ...
    try:
        ...
        v5_rows = []
    _p(f"V5: collect done rows={len(v5_rows)} dt={( _t.perf_counter()-t):.2f}s")
    ...
    _p(f"SQL: df_all/df_cand done shape={df_all.shape} dt={( _t.perf_counter()-t):.2f}s")
    ...
    _p(f"rename/round: done dt={( _t.perf_counter() - t_rename ): .2f}s")
    ...
    _p(f"RS/GrowthBias: done dt={( _t.perf_counter() - t_rs ): .2f}s")
    ...
    try:
        ...
        t_news = _t.perf_counter()

        def _news_apply_row(r):
            c = str(r.get("コード") or "").zfill(4)
            n = str(r.get("銘柄名") or "")
            return kabutan_news_line(c, n)

        news_ser_all = None

        # 先に df_all だけでニュースをまとめて実行
        if 'コード' in df_all.columns and '銘柄名' in df_all.columns:
            news_ser_all = kabutan_news_lines_bulk_for_dataframe(
                df_all, code_col='コード', name_col='銘柄名'
            )
            df_all['株探ニュース(3)'] = news_ser_all

        # df_cand 側は df_all の結果を「コード」でマッピングして流用
        if 'コード' in df_cand.columns and '銘柄名' in df_cand.columns:
            if news_ser_all is not None:
                # コード→ニュース の対応表を作る（重複コードは最後の行を採用）
                news_map = (
                    df_all[['コード', '株探ニュース(3)']]
                    .drop_duplicates(subset=['コード'], keep='last')
                    .set_index('コード')['株探ニュース(3)']
                )
                df_cand['株探ニュース(3)'] = df_cand['コード'].map(news_map).fillna("")
            else:
                # 保険：df_all のニュース取得に失敗したときだけ個別に取り直す
                df_cand['株探ニュース(3)'] = kabutan_news_lines_bulk_for_dataframe(
                    df_cand, code_col='コード', name_col='銘柄名'
                )
        _p(f"news: kabutan bulk done dt={( _t.perf_counter() - t_news ): .2f}s")
    except Exception as _e_news:
        print('[news][WARN] 株探ニュース列の付与に失敗:', _e_news)
    # === [/INJECTED] ===

    # 5) ログ（省略可）
    t_log = _t.perf_counter()
    ...
    if include_log:
        ...
        df_log = pd.DataFrame(columns=["日時","コード","種別","詳細"])
    _p(f"log: fetch done shape={df_log.shape} dt={( _t.perf_counter() - t_log ): .2f}s")

    
    # 6) フラグ装飾（略式）
    t_flags = _t.perf_counter()
    ...
    _p(f"flags: decorate done dt={( _t.perf_counter() - t_flags ): .2f}s")
    ...
    _t0 = _t.time()
    def _tick(msg):
        print(f"[timer] {msg}: {_t.time()-_t0:.2f}s", flush=True)
    ...
    try:
        ...
        build_id = dtm.datetime.now(_tz).strftime("%Y-%m-%d %H:%M:%S") if _tz else dtm.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    except Exception:
    ...
    t_inject = _t.perf_counter()
    html = __inject_inline_data_json(html, data_obj)
    _p(f"inject_inline_data_json: dt={( _t.perf_counter() - t_inject ): .2f}s")
...
def update_operating_income_and_ratio(conn, batch_size=300, max_workers=12, use_quarterly=False):
    ...
    ※方針A版：
      yahooquery には行かず、pl_quarter テーブルから
    ...
    try:
        ...
        print("[oper][WARN] pl_quarter 読み込みに失敗したためスキップ:", e)
        return

    if pl.empty:
        print("[oper] pl_quarter が空のためスキップ")
        return

    # 日付として扱うためのソートキー
    pl["決算期_ord"] = pd.to_datetime(pl["決算期"], errors="coerce")
    pl = pl.dropna(subset=["決算期_ord"])

    if pl.empty:
        print("[oper] pl_quarter に有効な決算期が無いためスキップ")
        return

    # コード＋決算期＋四半期順でソート（古い→新しい）
    pl = pl.sort_values(["コード", "決算期_ord", "四半期"])

    # 2) コードごとに「直近4レコード分の営業利益合計」を計算
    #    ※四半期データがそろっていない銘柄はあるので、その場合はある分だけの合計
    def _sum_last4(group):
        g = group.tail(4)
        # 営業利益が NaN のものは 0 扱い（完全に無い銘柄は後で弾く）
        vals = g["営業利益"].dropna()
        if vals.empty:
            return None
        return float(vals.sum())

    series_sum = pl.groupby("コード").apply(_sum_last4).rename("営業利益_sum")

    # None（有効な営業利益が取れなかった銘柄）は除外
    series_sum = series_sum.dropna()
    if series_sum.empty:
        print("[oper] 有効な営業利益が計算できずスキップ")
        return

    df_oper = series_sum.reset_index()

    # 3) 単位変換：百万円 → 億円（想定）
    #    ※もし pl_quarter.営業利益 が既に「億円」なら divisor を 1.0 に変更してください。
    DIVISOR_TO_OKU = 1.0

    def _to_oku(x):
        try:
            return float(x) / DIVISOR_TO_OKU
        except Exception:
            return None

    df_oper["営業利益_億"] = df_oper["営業利益_sum"].apply(_to_oku)
    ...
    df_oper["営業利益_億_int"] = df_oper["営業利益_億"].apply(_round_int)

    # 4) screener 側の時価総額を読み込み
    try:
        ...
        print("[oper][WARN] screener 読み込みに失敗したためスキップ:", e)
        return

    if sc.empty:
        print("[oper] screener が空のためスキップ")
        return
    ...
    def _calc_ratio(row):
        op_oku = row.get("営業利益_億_int")
        mc_oku = row.get("時価総額億円")
        try:
            if op_oku is None:
                return None
            if mc_oku is None or float(mc_oku) <= 0:
                return None
    ...
    print(f"[oper] 営業利益・営利対時価を更新しました rows={len(updates)}（pl_quarterベース）")
...
def run_fundamental_daily(force: bool = False):
    ...
    try:
        ...
        # ★ ここを修正：python 固定 → _sys.executable
        subprocess.run([_sys.executable, str(FUND_SCRIPT)], check=True)
...
def build_earnings_edge_scores(conn, lookback_n=6, follow_days=5, since="2022-01-01"):
    ...
    # dtype安定化（コードはstring）
    ev["コード"] = _norm_code_str(ev["コード"])
    ev["ev_date"] = pd.to_datetime(ev["ev_date"], errors="coerce")

    codes = ev["コード"].dropna().unique().tolist()
    if not codes:
        return pd.DataFrame(columns=["コード", "edge_score"])
    ...
    # dtype安定化
    ph["コード"] = _norm_code_str(ph["コード"])
    ph["日付"] = pd.to_datetime(ph["日付"], format="%Y-%m-%d", errors="coerce")
    for c in ["始値", "高値", "安値", "終値"]:
        ph[c] = pd.to_numeric(ph[c], errors="coerce")

    outs = []
    # groupbyキーもstring dtypeで揃っている
    for code, g_ev in ev.groupby("コード", sort=False):
        g_ph = ph.loc[s_eq_scalar(ph["コード"], code)].dropna(subset=["日付"]).sort_values("日付").reset_index(drop=True)
        if g_ph.empty:
            continue
        g_ph["idx"] = np.arange(len(g_ph), dtype=np.int32)

        # 直近N回＋α
        g_ev = g_ev.head(max(lookback_n, 6))
        rets, follows, wins = [], [], []
        ...
        for _, row in g_ev.iterrows():
            ...
            if pd.isna(d0):
                continue
            # 事前にboolean mask → idx最小の行を取る（高速）
            after_mask = g_ph["日付"] > d0
            if not after_mask.any():
                continue
            d1_idx = int(g_ph.loc[after_mask, "idx"].iloc[0])
            if d1_idx == 0:
                continue
            d0_idx = d1_idx - 1

            prev_close = g_ph.at[d0_idx, "終値"]
            next_close = g_ph.at[d1_idx, "終値"]
            if pd.isna(prev_close) or pd.isna(next_close) or prev_close == 0:
                continue
            ...
            hi_slice = g_ph.loc[d1_idx: d1_idx + max(1, follow_days) - 1, "高値"]
            hi_val = None if hi_slice.empty else float(pd.to_numeric(hi_slice, errors="coerce").max())
            follow_hi_pct = None if (hi_val is None or np.isnan(hi_val) or next_close == 0) else ((hi_val / next_close) - 1.0) * 100.0
    ...
    out_df = pd.DataFrame(outs, columns=["コード", "edge_score"])
    # 結合先の安定のためdtype固定
    out_df["コード"] = _norm_code_str(out_df["コード"])
    out_df["edge_score"] = pd.to_numeric(out_df["edge_score"], errors="coerce").fillna(0.0)
    return out_df
...
def build_pre_earnings_rank(conn: sqlite3.Connection) -> pd.DataFrame:
    ...
    if edge.empty:
        edge = pd.DataFrame(columns=["コード", "edge_score"])
    edge["コード"] = _norm_code_str(edge.get("コード", pd.Series([], dtype="string")))
    edge["edge_score"] = pd.to_numeric(edge.get("edge_score", 0.0), errors="coerce").fillna(0.0)
    ...
    if s.empty:
        s = pd.DataFrame(columns=["コード", "銘柄名", "右肩上がりスコア"])
    s["コード"] = _norm_code_str(s.get("コード", pd.Series([], dtype="string")))
    s["銘柄名"] = _str_series(s.get("銘柄名", pd.Series([], dtype="string")))
    s["右肩上がりスコア"] = pd.to_numeric(s.get("右肩上がりスコア", 0), errors="coerce")
    ...
    if hi.empty:
        ...
        hi["コード"] = _norm_code_str(hi["コード"])
        hi["close"] = pd.to_numeric(hi["close"], errors="coerce")
        hi["hh60"]  = pd.to_numeric(hi["hh60"], errors="coerce")
        with np.errstate(divide="ignore", invalid="ignore"):
            hi["near_hh"] = (hi["close"] / hi["hh60"] - 1.0) * 100.0
        hi["near_hh_score"] = hi["near_hh"].apply(
            lambda x: 100.0 if (pd.notna(x) and x >= -1.0) else (50.0 if (pd.notna(x) and x >= -5.0) else 0.0)
        )
    ...
    vol = pd.read_sql_query(
        ...
        v AS(
          SELECT p.コード, p.出来高 AS v0,
                 (SELECT AVG(出来高) FROM price_history q
                  WHERE q.コード=p.コード AND q.日付>=date(p.日付,'-20 day') AND q.日付<p.日付) AS v20
          FROM price_history p
    ...
    if vol.empty:
        ...
        vol["コード"] = _norm_code_str(vol["コード"])
        vol["v0"] = pd.to_numeric(vol["v0"], errors="coerce")
        vol["v20"] = pd.to_numeric(vol["v20"], errors="coerce")
        # v20が0/NaNの時は1.0扱い
        base = vol["v20"].where(vol["v20"].fillna(0) != 0, 1.0)
        with np.errstate(divide="ignore", invalid="ignore"):
            vol["boost"] = vol["v0"] / base
        vol["vol_score"] = vol["boost"].apply(
            lambda x: 100.0 if (pd.notna(x) and x >= 3.0) else
                      (70.0  if (pd.notna(x) and x >= 2.0) else
                      (40.0  if (pd.notna(x) and x >= 1.3) else 0.0))
        )

    # 結合（コード=stringで統一）
    df = s.merge(edge, on="コード", how="left") \
          .merge(hi[["コード", "near_hh_score"]], on="コード", how="left") \
          .merge(vol[["コード", "vol_score"]], on="コード", how="left")

    df["edge_score"] = pd.to_numeric(df.get("edge_score", 0), errors="coerce").fillna(0.0)
    mom_rightup = pd.to_numeric(df.get("右肩上がりスコア", 0), errors="coerce").fillna(0.0).clip(0, 100)
    near_hh     = pd.to_numeric(df.get("near_hh_score", 0), errors="coerce").fillna(0.0).clip(0, 100)
    vol_score   = pd.to_numeric(df.get("vol_score", 0), errors="coerce").fillna(0.0).clip(0, 100)

    df["momentum_score"] = (mom_rightup * 0.6 + near_hh * 0.25 + vol_score * 0.15).clip(0, 100)
    df["pre_score"] = (0.6 * df["edge_score"] + 0.4 * df["momentum_score"]).round(1)

    # 表示用列の型
    df["銘柄名"] = _str_series(df.get("銘柄名", pd.Series([], dtype="string")))
    df = df.sort_values(["pre_score"], ascending=False)

    # printサンプル（必要なら残す）
    print("pre_earn sample:", df[["コード", "銘柄名", "pre_score"]].head(5).to_dict(orient="records"))
...
def build_earnings_tables(conn):
    ...
    have = set(_str_series(cols_df["name"]))
    base_cols = ["コード", "提出時刻", "タイトル", "センチメント"]
    opt_cols  = ["ヒットKW"]
    sel_cols  = [c for c in base_cols + opt_cols if c in have]
    ...
    if sel_cols:
        ...
        """
        ev_df = pd.read_sql_query(ev_sql, conn)
    else:
        ev_df = pd.DataFrame()

    if not ev_df.empty:
        ev_df["コード"] = _norm_code_str(ev_df["コード"])
        names = pd.read_sql_query("SELECT コード, 銘柄名 FROM screener", conn)
        if not names.empty:
            names["コード"] = _norm_code_str(names["コード"])
        ev_df = ev_df.merge(names, on="コード", how="left")

        ts = pd.to_datetime(ev_df.get("提出時刻"), errors="coerce")
        ev_df["時刻"] = ts.dt.strftime("%Y/%m/%d %H:%M").fillna(_str_series(ev_df.get("提出時刻", "")))
    ...
    if not pre_df.empty:
        pre_df["コード"] = _norm_code_str(pre_df["コード"])
        if "銘柄名" not in pre_df.columns or pre_df["銘柄名"].isna().any():
            names = pd.read_sql_query("SELECT コード, 銘柄名 FROM screener", conn)
            if not names.empty:
                names["コード"] = _norm_code_str(names["コード"])
...
def _yahoo_quote_url(code: str, market: str | None = None, conn: sqlite3.Connection | None = None) -> str:
    ...
    try:
        ...
        # 明示市場が無い/判定できないときは resolve_yahoo_symbol の結果から推定
        if not suf:
            try:
                ys = resolve_yahoo_symbol(raw, conn)  # may raise if not defined
...
def _timed(label, func, *args, **kwargs):
    ...
    try:
        ...
        dt = time.time() - t0
        print(f"[TIMER] {label}: {_fmt_hms(dt)}")
...
def _auto_run_mode():
    ...
    if dtm.time(9, 0) <= now < dtm.time(15, 25):
        return "MIDDAY"
    else:
        return "EOD"
# ===== メイン処理 =====
    ...
    while True:
        ...
        try:
            ...
            if kind == "sql":
                ...
                chunk = int(job.get("chunk") or 500)
                cur = conn.cursor()
                try:
                    if rows:
                        # chunked executemany
                ...
                fn = globals().get(name)
                if fn is None:
                    print(f"[writer][WARN] function not found: {name}")
                else:
                    # call with writer-owned connection
                    fn(conn)
            else:
                print(f"[writer][WARN] unknown job kind: {kind}")
        except Exception as e:
...
def start_writer():
    ...
    if _write_q is not None:
        return
    _write_q = JoinableQueue()
    _writer_proc = Process(target=_writer_loop, args=(_write_q,), daemon=True)
    _writer_proc.start()
...
def enqueue_sql(sql, params=None):
    start_writer()
    _write_q.put({"kind": "sql", "sql": sql, "params": params or []})

def enqueue_execmany(sql, rows, chunk=500):
    start_writer()
    _write_q.put({"kind": "execmany", "sql": sql, "rows": rows, "chunk": chunk})

def enqueue_call(func_name: str):
    start_writer()
    _write_q.put({"kind": "call", "name": func_name})
...
def _norm_code_str(code: object) -> str:
    import re as _re
    s = str(code).strip().upper()
    for suf in (".T", "-T", ".JP", "-JP"):
        if s.endswith(suf):
...
def _sanitize_codes(df: pd.DataFrame, col: str = "コード") -> pd.DataFrame:
    if col not in df.columns:
        # 形だけ返す（空）
        return pd.DataFrame(columns=[col])
    out = df.copy()
    out[col] = out[col].astype(object).map(_norm_code_str)
    out = out[out[col] != ""]
    return out
...
def build_pre_earnings_rank(conn: sqlite3.Connection) -> pd.DataFrame:
    ...
    """
    edge = build_earnings_edge_scores(conn)  # ["コード","edge_score"]
    edge = _sanitize_codes(edge, "コード")

    s = pd.read_sql_query(
        "SELECT コード, 銘柄名, 現在値, 前日終値比率, 出来高, 右肩上がりスコア FROM screener",
        conn
    )
    s = _sanitize_codes(s, "コード").drop_duplicates(subset=["コード"], keep="last")
    ...
    hi = pd.read_sql_query("""
      ...
      SELECT p.コード, p.終値 AS close,
             (SELECT MAX(高値) FROM price_history q
              WHERE q.コード=p.コード AND q.日付 >= date(p.日付,'-60 day')) AS hh60
      FROM price_history p
      JOIN z ON p.コード=z.コード AND p.日付=z.d
    """, conn)

    if not hi.empty:
        hi = _sanitize_codes(hi, "コード")
        # 0除算回避
        hi["hh60"] = hi["hh60"].replace({0: np.nan})
        hi["near_hh"] = (hi["close"] / hi["hh60"] - 1.0) * 100.0
        hi["near_hh_score"] = np.select(
            [hi["near_hh"] >= -1.0, hi["near_hh"] >= -5.0],
            [100.0, 50.0],
            default=0.0
        )
        hi = hi[["コード","near_hh_score"]]
    else:
        hi = pd.DataFrame(columns=["コード","near_hh_score"])
    ...
    vol = pd.read_sql_query("""
      ...
      SELECT コード, v0, v20 FROM v
    """, conn)

    if not vol.empty:
        vol = _sanitize_codes(vol, "コード")
        vol["boost"] = vol.apply(lambda r: (r["v0"] / max(1.0, r["v20"])) if r["v20"] else 1.0, axis=1)
        vol["vol_score"] = np.select(
            [vol["boost"] >= 3.0, vol["boost"] >= 2.0, vol["boost"] >= 1.3],
            [100.0, 70.0, 40.0],
            default=0.0
        )
        vol = vol[["コード","vol_score"]]
    else:
        vol = pd.DataFrame(columns=["コード","vol_score"])

    df = s.merge(edge, on="コード", how="left")           .merge(hi,   on="コード", how="left")           .merge(vol,  on="コード", how="left")

    df["edge_score"] = df["edge_score"].fillna(0.0)
    df["右肩上がりスコア"] = df["右肩上がりスコア"].fillna(0.0).clip(0, 100)
    df["near_hh_score"]  = df["near_hh_score"].fillna(0.0).clip(0, 100)
    df["vol_score"]      = df["vol_score"].fillna(0.0).clip(0, 100)

    df["momentum_score"] = (df["右肩上がりスコア"] * 0.6 +
                            df["near_hh_score"]  * 0.25 +
                            df["vol_score"]      * 0.15).clip(0, 100)

    df["pre_score"] = (0.6 * df["edge_score"] + 0.4 * df["momentum_score"]).round(1)

    out_cols = ["コード","銘柄名","pre_score","edge_score","momentum_score"]
    if not set(out_cols).issubset(df.columns):
        missing = [c for c in out_cols if c not in df.columns]
        print(f"[preearn][WARN] missing columns {missing}, available={list(df.columns)[:10]}...")
        df = pd.DataFrame(columns=out_cols)

    df = df[out_cols].dropna(subset=["コード"])
    df = _sanitize_codes(df, "コード").drop_duplicates(subset=["コード"], keep="last")
    df = df.sort_values(["pre_score","edge_score","momentum_score"], ascending=False)

    bad_mask = df["コード"].str.len() > 8
    if bad_mask.any():
        cnt = int(bad_mask.sum())
        sample_bad = df.loc[bad_mask, ["コード"]].head(3).to_dict(orient="records")
        print(f"[preearn][SANITY][WARN] invalid code length rows={cnt} sample={sample_bad}")
        df = df.loc[~bad_mask]
...
def build_earnings_tables(conn):
    ...
    if not ev_df.empty:
        ev_df = _sanitize_codes(ev_df, "コード")
        names = pd.read_sql_query("SELECT コード, 銘柄名 FROM screener", conn)
        names = _sanitize_codes(names, "コード")
        ev_df = ev_df.merge(names, on="コード", how="left")
        ts = pd.to_datetime(ev_df.get("提出時刻"), errors="coerce")
    ...
    pre_df = build_pre_earnings_rank(conn)
    if not pre_df.empty:
        pre_df = _sanitize_codes(pre_df, "コード")
...
def _apply_planA_postprocess(html_path, data_obj):
    try:
        with open(html_path, "r", encoding="utf-8") as _rf:
            _html = _rf.read()
        _html2 = _singlefile_planA_transform(_html, data_obj)
        if _html2 != _html:
            with open(html_path, "w", encoding="utf-8", newline="") as _wf:
                _wf.write(_html2)
            print("[singlefile][PlanA] postprocess applied")
        else:
            print("[singlefile][PlanA] no changes made")
    except Exception as _e:
        print("[singlefile][PlanA][ERROR]", _e)

import re as _re, json as _json, html as _html_mod

def _singlefile_planA_transform(html_str, data_obj):
    # 1) Neutralize only data.js script tags
    def _kill_data_js(m):
        s = m.group(0)
        return '<!-- __neutralized_data_js__ ' + _html_mod.escape(s) + ' -->'
    html_str = _re.sub(
        r'<script[^>]+src=["\\\']([^"\\\']*data\\.js[^"\\\']*)["\\\'][^>]*>\\s*</script>',
        _kill_data_js, html_str, flags=_re.I
    )
    # 2) Inject __DATA__ inline (after </nav> if exists, otherwise after <body>)
    try:
        data_json = _json.dumps(data_obj, ensure_ascii=False, separators=(",",":"))
    except Exception:
        # Fallback: try dict(...) if it's a pandas-friendly thing
        try:
            data_json = _json.dumps(dict(data_obj), ensure_ascii=False, separators=(",",":"))
        except Exception:
            data_json = "{}"
    ...
    if '</nav>' in html_str:
        ...
        html_str = _re.sub(r'<body([^>]*)>', r'<body\\1>\\n'+data_script, html_str, count=1, flags=_re.I)
    return html_str
# ===== End Plan A helpers =====
