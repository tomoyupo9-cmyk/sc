...
def _str_series(x):
    """配列/Seriesを安全に文字列Series化（NA→""）"""
    return pd.Series(x, dtype=str).fillna("")

def _norm_code_series(arr):
    """Seriesまたはリストに対し、各要素を正規化して返す"""
...
def dumps_json_clean(obj,
    ...
    try:
        ...
        try:
            ...
            pass
        bs = orjson.dumps(obj, default=_coerce, option=opt)
        s = bs.decode("utf-8")
        if ensure_ascii:
...
def resolve_yahoo_symbol(code: str,
    ...
    if pat_3d1a.match(S) or (len(S) == 4 and pat_4_alnum.match(S) and not S.isdigit()):
        if try_online:
            ...
            try:
                for cand in (s + ".T", s):
                    df = yfinance.download(cand, period="5d", interval="1d", progress=False)
                    if not df.empty:
                        return cand
            except Exception:
...
def _get_last_two_closes(conn, code: str):
    ...
    try:
        ...
        for i in range(1, len(df)):
            d = pd.to_datetime(df.iloc[i]["日付"]).date()
            if d != cur_date:
                prev_date = d
...
def _vectorize_minimum_fields(df):
    ...
    if "コード" in df.columns:
        try:
            c4 = pd.Series(df["コード"], copy=False).astype(str).str.strip().str.zfill(4)
            df["c4"] = c4
        except Exception:
            pass
    else:
...
def apply_squeeze_labels(df: pd.DataFrame) -> pd.DataFrame:
    """
    Squeeze係数をパーセンタイルランク化してラベル(●●●)を付与する関数
    add_price_features の後に呼び出すこと。
    """
    ...
    # ランク算出（NaNは除外）
    # 値が小さいほど収束している（＝エネルギーが溜まっている）ため ascending=True
    df["Squeeze_Rank"] = df["Squeeze"].rank(pct=True, ascending=True)
    
    # ラベル付与
    conditions = [
        (df["Squeeze_Rank"] <= 0.10),
        (df["Squeeze_Rank"] <= 0.25),
        (df["Squeeze_Rank"] <= 0.50)
    ]
    choices = ['●●●', '●●○', '●○○']
    df["Squeeze_Label"] = np.select(conditions, choices, default='○○○')
    
    return df

def add_price_features(df):
    """
    価格系列の典型指標をベクトル化で一括付与するテンプレ関数（Squeeze係数対応版）
    """
    if df.empty:
        return df
    # 防御: 必須列のみで進める
    ...
    # --- Volatility Squeeze (20日) の計算 ---
    # std20: 20日標準偏差
    std20 = grp["終値"].transform(lambda s: s.rolling(20, min_periods=20).std())
    # Squeeze = (UpperBand - LowerBand) / MA20 = (4 * std20) / MA20
    # ここでは計算を簡略化し、Squeeze係数として (4 * std20) / ma20 を保持
    df["Squeeze"] = (4 * std20) / df["終値_ma20"]
    # ----------------------------------------
...
def update_market_cap_all(conn, batch_size=300, max_workers=8):
    ...
    try:
        ...
        for c in codes:
            try:
                #fi = yfinance.YQ(_to_symbol(c)).fast_info
                fi = yfinance.YQ(resolve_yahoo_symbol(c, conn, True)).fast_info
...
def _prepare_rows_fast(df: pd.DataFrame, conn):
    ...
    for r in _df.itertuples(index=False):
        ...
        try:
            d["x_url"] = (f"https://x.com/search?q={quote(str(name_val))}" if name_val else None)
        except Exception:
...
def _kabunews_fetch(code: str, name: str) -> list[dict]:
    ...
    try:
        ...
        r = requests.get(url, headers=headers, timeout=_KABUNEWS_CONF["http_timeout"])
        ct = r.headers.get("Content-Type", "")
        if dbg:
            print(
                f"[kabunews] RESP {key} status={r.status_code} ct='{ct}' "
                f"bytes={len(r.content)} dt={(time.perf_counter()-t0):.2f}s",
                flush=True
            )
        r.raise_for_status()
    ...
    try:
        for it in root.findall(".//item"):
            ...
            if title and link:
                items.append({"title": html.unescape(title), "url": link, "pub": pub})
        ...
        try:
            ...
            if dbg:
                print(
                    ...
                    f"dt={(time.perf_counter()-bert_t0):.2f}s",
                    flush=True
                )
...
def kabutan_news_fetch_bulk(pairs, per_symbol=None):
    ...
    while i < len(pairs):
        ...
        if len(url) > 1800 and len(chunk) > 1:
            ...
            continue
        try:
            resp = requests.get(url, headers={"User-Agent": _KABUNEWS_CONF["user_agent"]}, timeout=_KABUNEWS_CONF["http_timeout"])
...
def kabutan_news_lines_bulk_for_dataframe(df, code_col="コード", name_col="銘柄名", per_symbol=None):
    ...
    def _fmt_pub(pub: str) -> str:
        ...
        for pat in ("%a, %d %b %Y %H:%M:%S %Z", "%a, %d %b %Y %H:%M:%S %z"):
            try:
                ...
                return datetime.strftime("%y/%m/%d")  # ← 25/11/14 みたいな形
            except Exception:
                pass
    ...
    return pd.Series(data, index=df.index, dtype=object)
    ...
    try:
        ...
        for tr in table.find_all("tr"):
            ...
            for a in tr.find_all("a", href=True):
                href = a["href"] or ""
                m = re.search(r"code=(\d{4}[A-Z]?)", href)
                if not m:
                    m = re.search(r"/stock/(\d{4}[A-Z]?)", href)
                if not m:
                    continue
                codes.add(m.group(1))
...
def eod_refresh_recent_3days(conn, batch_size: int = 60):
    ...
    try:
        ...
        for i in range(0, len(codes), batch_size):
            ...
            try:
                df_wide = yfinance.download(list(tickers_map.values()), period="10d", interval="1d",
                                      group_by="ticker", threads=True, auto_adjust=False)
            except Exception as e:
...
def gap_patrol_recent_15(conn, batch_size: int = 60):
    ...
    for i in range(0, len(codes), batch_size):
        ...
        tickers_map = {c: resolve_yahoo_symbol(str(c), conn, True) for c in chunk}
        try:
            df_wide = yfinance.download(list(tickers_map.values()), period="30d", interval="1d",
                                  group_by="ticker", threads=True, auto_adjust=False)
        except Exception as e:
...
def _apply_planA_postprocess(html_path, data_obj):
    try:
        with open(html_path, "r", encoding="utf-8") as _rf:
            html = _rf.read()
        _html2 = _singlefile_planA_transform(html, data_obj)
        if _html2 != html:
