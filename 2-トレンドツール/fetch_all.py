...
function render(j){
  ...
  if(MODE==="bbs"){
    ...
    rows.forEach((r,idx)=>{
      ...
      t(offeringCell(r.has_offering_history)); // 増資経歴
      t(String(b24));                          // BBS(24h)
      t(String(b72));                          // BBS(72h)
      t(String(growth));                       // 増加率
      t(badge(label));                         // Sentiment(News)
      t(toNewsCards(items));                   // 最新ニュース
      tbody.appendChild(tr);
    });
    return;
  }

  // 共通: 検索系（漏れてる？ / TOB）レンダラ
  ...
  rows.forEach((r,idx)=>{
    ...
    t(String(idx+1));
    t(`${esc(r.name||r.ticker)} <span class="code">(${esc(r.ticker)})</span>`);
    t(offeringCell(r.has_offering_history));          // ★ 追加：増資経歴
    t(fmt(r.score));
...
def _extract_text_pdfminer(pdf_bytes: bytes) -> str:
    try:
        from pdfminer_high_level import extract_text  # some envs use pdfminer.high_level
    except Exception:
        try:
...
def tdnet_items_to_earnings_rows(tdnet_items):
    ...
    # ←← ここが修正（インデント4スペース＆returnを含む）
    # parallel PDF summarize for top-N
    try:
...
def _summarize_one_pdf_row(row: dict) -> dict:
    ...
    try:
        ...
        if not b:
            return row
        try:
            text = _extract_text_from_pdf(b)  # expects existing helper
        except NameError:
            return row
        if not text:
            return row
...
def upsert_earnings_rows_fast(conn, rows):
    ...
    try:
        # ★ 正：元の upsert_earnings_rows() を呼ぶ（自分自身は呼ばない）
