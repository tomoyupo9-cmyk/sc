...
def fetch_search_comments(db_path: str, query: str, query_key: str,
    ...
    if stop_on_known:
        for p, url in enumerate(page_urls, start=1):
            r = _http_get(url); 
            if not r: break
            comments, empty = _parse_search_page(r.text, enc)
            new, reached = [], False
            for c in comments:
                if c["link"] in existing:
                    reached = True; break
                new.append(c)
            if new: page_results.append((p, new))
            if empty or reached: break
    else:
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            fut2p = {ex.submit(_http_get, u):(i+1) for i,u in enumerate(page_urls)}
            for fut in as_completed(fut2p):
                p = fut2p[fut]; r = fut.result()
                if not r: continue
                comments, _ = _parse_search_page(r.text, enc)
                comments = [c for c in comments if c["link"] not in existing]
                if comments: page_results.append((p, comments))
